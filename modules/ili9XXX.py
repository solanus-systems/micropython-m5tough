...
class ili9XXX:
    ...
    def __init__(
        ...
        if not color_format:
            raise RuntimeError(
                f"No color format defined for display {self.display_name}"
            )
        ...
        if not self.buf1:
            ...
            raise RuntimeError(
                f"Not enough DMA-capable memory to allocate display buffer. Needed: {self.buf_size} bytes, largest free block: {free} bytes"
            )
        self.buf2 = (
            esp.heap_caps_malloc(self.buf_size, _MALLOC_DMA) if double_buffer else None
        )
    ...
    def disp_spi_init(self):
        ...
        devcfg = esp.spi_device_interface_config_t(
            {
                "clock_speed_hz": self.mhz
                * 1000
                * 1000,  # Clock out at DISP_SPI_MHZ MHz
        ...
        if buscfg.mosi_io_num >= 0 and buscfg.sclk_io_num >= 0:
    ...
    def deinit(self):
        ...
        if self.spi:
            ...
            ret = 0
            while ret == 0:
                ret = esp.spi_device_get_trans_result(
                    self.spi, self.trans_result_ptr, 1
                )
    ...
    def send_data(self, data):
        ...
        if len(data) > _TRANSFER_BUFFER_LENGTH:
    ...
    def flush(self, disp_drv, area, color_p):
        if (
            self.end_time_ptr.int_val
            and self.end_time_ptr.int_val > self.start_time_ptr.int_val
        ):
            self.flush_acc_dma_cycles += (
...
class ili9341(ili9XXX):
    def __init__(
        ...
        if lv.color_format_get_bpp(color_format) != 16:
            raise RuntimeError(
                "ili9341 micropython driver requires 16 bit color format"
            )
...
class ili9488(ili9XXX):
    def __init__(
        ...
        if (lv.color_format_get_bpp(color_format) != 32) and not p16:
            raise RuntimeError(
                "ili9488 micropython driver requires 32 bit color format"
            )
        if not hybrid:
            raise RuntimeError(
                "ili9488 micropython driver do not support non-hybrid driver"
            )
...
class gc9a01(ili9XXX):
    ...
    def __init__(
        ...
        self.init_cmds = [
            ...
            {
                "cmd": 0x70,
                "data": bytes([0x07, 0x07, 0x04, 0x0E, 0x0F, 0x09, 0x07, 0x08, 0x03]),
            },
        ...
        super().__init__(
            ...
            double_buffer=double_buffer,
            half_duplex=half_duplex,
            display_type=DISPLAY_TYPE_GC9A01,
            asynchronous=asynchronous,
            initialize=initialize,
            color_format=color_format,
            swap_rgb565_bytes=swap_rgb565_bytes,
...
class st7735(ili9XXX):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            double_buffer=double_buffer,
            half_duplex=half_duplex,
            display_type=DISPLAY_TYPE_ST7735,
            asynchronous=asynchronous,
            initialize=initialize,
            color_format=color_format,
            swap_rgb565_bytes=swap_rgb565_bytes,

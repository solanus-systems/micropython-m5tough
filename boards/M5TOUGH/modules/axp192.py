...
class AXP192:
    ...
    def twiddle(self, reg_addr, affects, value):
        self.write_byte(
            reg_addr, (self.read_byte(reg_addr) & (affects ^ 0xFF)) | (value & affects)
        )
    ...
    def read(self, reg_addr, length=1):
        ...
        if reg_addr == ACIN_VOLTAGE or reg_addr == VBUS_VOLTAGE:
            ...
            # 1.1mV * 0.5mA per LSB
            return int.from_bytes(self.read(BATTERY_POWER, 3), "big") * (
                1.1 * 0.5 / 1000
            )
    ...
    def coulomb_counter(self):
        ...
        return (
            32768
            * (self.read(CHARGE_COULOMB) - self.read(DISCHARGE_COULOMB))
            / 3600
            / 25
        )
